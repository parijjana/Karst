...
class QueryService:
    ...
    @staticmethod
    def _error(
        code: QueryErrorCode, message: str, *, retryable: bool = False
    ) -> ApiError:
    ...
    def list_symbols(
        ...
        try:
            ...
            return SymbolPageError(
                error=self._error(QueryErrorCode.INVALID_CURSOR, "Cursor is invalid.")
            )
        except StaleCursorError:
            return SymbolPageError(
                error=self._error(QueryErrorCode.STALE_CURSOR, "Cursor is stale.")
            )
            ...
            if message == "Index is not ready.":
                return SymbolPageError(
                    error=self._error(
                        QueryErrorCode.INDEX_NOT_READY, message, retryable=True
                    )
                )
            if message == "Limit is invalid.":
                return SymbolPageError(
                    error=self._error(QueryErrorCode.LIMIT_EXCEEDED, message)
                )
            return SymbolPageError(
                error=self._error(
                    QueryErrorCode.INDEX_NOT_READY,
            ...
            return SymbolPageError(
                error=self._error(
                    QueryErrorCode.LIMIT_EXCEEDED, "Query parameters are invalid."
                )
            )
        return SymbolPageSuccess(data=page)

    def get_snippet(
        self, project_id: int, stable_symbol_id: str
    ) -> SnippetSuccess | SnippetError:
        try:
            ...
            if message == "Symbol not found.":
                return SnippetError(
                    error=self._error(QueryErrorCode.SYMBOL_NOT_FOUND, message)
                )
            if message == "Source is stale.":
                return SnippetError(
                    error=self._error(
                        QueryErrorCode.SOURCE_STALE, message, retryable=True
                    )
                )
            if message == "Index is not ready.":
                return SnippetError(
            ...
            return SnippetError(
                error=self._error(QueryErrorCode.SYMBOL_NOT_FOUND, "Symbol is invalid.")
            )
