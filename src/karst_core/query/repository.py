...
class SymbolRepository:
    ...
    def __init__(
        self,
        database: Any,
        cursor_codec: SymbolPageCursorCodec,
        *,
        snippet_utf8_budget: int = MAX_SNIPPET_UTF8_BYTES,
        max_source_bytes: int = 4 * 1024 * 1024,
    ) -> None:
        if (
            not isinstance(snippet_utf8_budget, int)
            or not 1 <= snippet_utf8_budget <= MAX_SNIPPET_UTF8_BYTES
        ):
    ...
    def list_symbols(
        ...
        if cursor is not None:
            try:
                after = self.codec.decode(
                    cursor,
                    expected_project_stable_id=project_stable_id,
        ...
        if after is not None:
            clauses.append(
                "(file.relative_path, node.start_line, node.qualified_name, node.stable_id) > (?, ?, ?, ?)"
            )
        ...
        items = tuple(
            SymbolRef(
                stable_symbol_id=str(r[0]),
                kind=str(r[1]),
                name=str(r[2]),
                qualified_name=str(r[3]),
                signature=None if r[4] is None else str(r[4]),
                relative_path=str(r[5]),
                start_line=int(r[6]),
                end_line=int(r[7]),
                source_sha256=str(r[8]),
        ...
        if len(rows) > limit:
            ...
            next_cursor = self.codec.encode(
                ...
                after=SymbolPageKeyset(
                    ...
                    last.start_line,
                    last.qualified_name,
                    last.stable_symbol_id,
                ),
            )
        return SymbolPage(
            items=items, next_cursor=next_cursor, generation_id=generation
        )
...
__all__ = ["QueryRepositoryError", "SymbolRepository"]
