"""Read-only structural graph for Karst clients.
...
class StructuralGraphService:
    ...
    def graph(
        ...
        if not self._db_path.exists():
            return self._apply_folder_focus(StructuralGraph([], []), selected_folder_id)
    ...
    def _apply_folder_focus(
        ...
        selected = next(
            (
                ...
                if node.get("id") == selected_folder_id and node.get("type") == "folder"
            ),
            None,
        )
        if selected is None:
        ...
        focus_ids.update(
            str(node["id"])
            for node in graph.nodes
            if node.get("type") == "code_dot" and node.get("parent_id") in focus_ids
        )
        focused_nodes = [
            {
                **node,
    ...
    def _append_project(
        ...
        folder_ids = {
            folder: _opaque_id("folder", str(project["stable_id"]), folder)
            for folder in folders
        }
        ...
        for file in files:
            ...
            links.append(
                {"source": parent_id, "target": file_key, "type": "structural"}
            )
...
def _ancestor_ids(
    ...
    return [
        "karst",
        project_id,
        *[folder_ids["/".join(parts[:index])] for index in range(1, len(parts) + 1)],
    ]
