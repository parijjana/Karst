...
# Stable response models owned by the reusable Karst query core.
...
def _safe_message(value: str) -> str:
    message = require_utf8_text(value, "message", MAX_ERROR_MESSAGE_UTF8_BYTES)
    if (
        "/" in message
        or "\\" in message
...
class SymbolRef(_ContractModel):
    stable_symbol_id: str = Field(min_length=36, max_length=36, pattern=_UUID5_PATTERN)
    kind: str = Field(
        min_length=1,
    ...
    source_sha256: str = Field(min_length=64, max_length=64, pattern=_SHA256_PATTERN)
...
class SymbolPage(_ContractModel):
    ...
    @classmethod
    def _items_are_bounded(cls, value: tuple[SymbolRef, ...]) -> tuple[SymbolRef, ...]:
