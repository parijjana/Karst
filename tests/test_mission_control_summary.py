    ...
    assert ProjectSummaryService(settings.db_path).files(project_id, 10, 0) == [
        {
            "id": 1,
            "path": "module.py",
            "hash": summary_file_hash(settings),
            "nonblank_loc": 2,
        }
    ]
...
def test_changed_routes_use_core_summary_service_without_web_db_helper(
    tmp_path: Path,
) -> None:
    settings = _settings(tmp_path)
    project = tmp_path / "project"
    ...
    assert not hasattr(web_data, "get_db")
    assert not hasattr(web_data, "table_exists")
